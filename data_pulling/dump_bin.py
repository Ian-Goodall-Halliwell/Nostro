...
class DumpDataBase:
    ...
    def save_instruments(self, instruments_data: Union[list, pd.DataFrame]):
        ...
        #instruments_path2 = str(self._instruments_dir.joinpath("cci.txt").resolve())
        if isinstance(instruments_data, pd.DataFrame):
            ...
            for en,a in enumerate(instruments_data.values):
                if a[0] == 'CCI':
                    op = {"symbol":[a[0]],"start_datetime":[a[1]], "end_datetime":[a[2]]}
                    #instruments_data2 = pd.DataFrame(op)#,columns=list(instruments_data.columns.values))
                    instruments_data.drop(labels=en, axis=0)
                else:
                    continue
            #instruments_data2.to_csv(instruments_path2, header=False, sep=self.INSTRUMENTS_SEP, index=False)    
...
class DumpDataUpdate(DumpDataBase):
    def __init__(
        ...
        def try_get_name(process_instance):
            try:
                return pd.Timestamp(process_instance)
            except:
                return process_instance
        # load all csv files
        self._all_data = self._load_all_source_data()  # type: pd.DataFrame
        self._new_calendar_list = self._old_calendar_list + sorted(
            filter(lambda x: try_get_name(x) > self._old_calendar_list[-1], self._all_data[self.date_field_name].unique())
        )
